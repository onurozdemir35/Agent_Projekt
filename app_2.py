...
# Agent mit Memory initialisieren
agent_executor = create_react_agent(model=llm, tools=[retriever_tool], checkpointer=my_checkpointer)

st.title("🔍 Dokumenten-Chatbot mit Agent & Memory (persistiert)")

# Chatverlauf aus dem Memory laden (Liste von Messages)
chat_history = my_checkpointer.load()  # Dict mit "messages" Schlüssel
...
if query:
    with st.spinner("Suche Antwort..."):
        ...
        chat_history = my_checkpointer.load()
